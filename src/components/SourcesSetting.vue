<template>
  <div class="flex mb-3 pt-0">
    <input
      ...
      class="px-3 py-3 placeholder-gray-300 text-gray-600 relative bg-white text-xl border rounded-l-md border-gray-100 outline-none focus:outline-none focus: w-full"
    />
    <button
      @click="addSourceData"
      class="px-8 rounded-r-md bg-gray-200 transition-colors duration-150 text-gray-500 hover:text-gray-800 hover:bg-green-200 font-semibold p-4"
    >
      Add
    </button>
  </div>
          ...
          <div class="flex flex-col w-10 h-10 justify-center items-center mr-4">
            <a href="#" class="block relative">
              <img
                ...
                class="mx-auto object-cover rounded-full h-5 w-5"
              />
            </a>
          </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['sources', 'sourceAddingError']),
  },
  methods: {
    ...mapActions([
    ...
    getSourceFaviconURL(source) {
      return `/api/favicons/${source.faviconUUID || 'placeholder'}.png`
    },
  },
}
</script>
