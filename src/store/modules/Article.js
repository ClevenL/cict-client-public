...
const getters = {
    filteredArticles: (state) => {
        if (state.selectedSources.length && !state.search.length)
            return filters[state.type](state.articles).filter((article) =>
                state.selectedSources.some(
                    (title) => title == article.sources.title
                )
            )
    ...
    unReadArticles: (state) => {
        return state.articles.filter(
            (article) => !article.hide && !article.read
        )
    },
...
const actions = {
    ...
    async setArticleModal({ commit, dispatch }, data) {
        ...
        commit(
            ...
            await api.getArticleContent(
                encodeURIComponent(utils.prettyURL(data))
            )
        )
    },
