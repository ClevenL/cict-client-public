<template>
  ...
  <div class="flex flex-col flex-1">
    ...
    <main class="h-full overflow-y-auto">
      <!-- Content goes here -->
      <div class="max-w-5xl px-8 py-4 my-10 shadow-md mx-auto bg-white">
        <div v-if="settingsMenuState == 'Categories'" >  
...
export default {
  ...
  methods: {
    ...
    addSources() {
      sourcesArray.forEach(arraySource => {
        if (!this.sources.find(source => source.title == arraySource.title)) this.addSource({rss_url: arraySource.rss_url})        
      })
    },
     /*
    addCategories() {
      categoriesArray.forEach(arrayCategory => {
        if (!this.categories.find(category => category.title == arrayCategory.title)) this.addCategory(arrayCategory)
      })
    },
    */
    ...
    async saveSelection() {
      this.sources.forEach(async source => {
        if(this.checkedArray.includes(source.title)){
          await axios.put("api/sources/" + source._id, {
            active: true
          });
        }
        else {
    ...
    async deleteAllCategories() {
      ...
      this.deleteAllCategoriesModal = false
      this.categories = []
    },
    async deleteAllSources() {
      await axios.delete("api/sources/delete/all");
      this.deleteAllSourcesModal = false
      this.sources = []
    },
    async deleteAllArticles() {
      await axios.delete("api/articles/delete/all");
      this.deleteAllArticlesModal = false
    },
  },
};
</script>
